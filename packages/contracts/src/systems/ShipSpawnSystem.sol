...
contract ShipSpawnSystem is System {
  ...
  function execute(bytes memory arguments) public returns (bytes memory) {
    (Coord memory location, uint32 rotation, uint32 length) = abi.decode(arguments, (Coord, uint32, uint32));
  ...
  function executeTyped(
    ...
    return abi.decode(execute(abi.encode(location, rotation, length)), (uint256));
  }
}
